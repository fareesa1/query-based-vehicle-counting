#Do you like to learn with hands-on projects? So this is a right place for you, Starting with a quick tour to Machine Learning, building models and web based appications with those models you will be taken to deep dive into computer vision with opencv also giving space for your ideas, explore more by solving assignments and capstone projects
import gui3
from tkinter import *
...
def callback():
	...
	global counter_motorbike
	global counter_train
	global counter_bicycle
	global writer
	global W
	global H
	global memory
	...
	weightsPath = gui3.coco_weights
	configPath = gui3.coco_cfg
	# weightsPath = os.path.sep.join([C:/, "yolov3.weights"])
...
# loop over frames from the video file stream
	...
	# Thickness of -1 will fill the entire shape 
		...
		total_vehicle = counter_bicycle
		total_vehicle += counter_motorbike
		total_vehicle += counter_car
		total_vehicle += counter_truck
		total_vehicle += counter_bus
		total_vehicle += counter_train
	...
	# Draw a rectangle of black color of thickness -1 px 
		...
		cv2.putText(frame, str(total_vehicle), (1175, 220), cv2.FONT_HERSHEY_DUPLEX, 0.5, (0, 0, 0), 1)

	# check if the video writer is None
		if writer is None:
			# initialize our video writer
			fourcc = cv2.VideoWriter_fourcc(*"MJPG")
			writer = cv2.VideoWriter(gui3.output_video, fourcc, 30,
...
button1 = tk.Button(root, text='Submit', command=callback).pack()
